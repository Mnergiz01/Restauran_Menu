<template>
  <div class="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 overflow-x-hidden">
      ...
      <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div class="flex justify-between h-14 sm:h-16 items-center">
          <div class="flex-shrink-0 flex items-center">
            <img v-if="settings.logo_url" :src="settings.logo_url" class="h-8 sm:h-9 w-auto"
              :alt="settings.restaurant_name" />
            <h1 v-else class="text-lg sm:text-xl font-bold truncate" :style="{ color: settings.primary_color }">{{
              settings.restaurant_name }}</h1>
          </div>
          <div>
    ...
    <div v-if="showSearch" class="md:hidden bg-white border-b border-gray-200 px-3 py-3 sticky top-14 sm:top-16 z-25">
      <div class="relative">
        <input v-model="searchQuery" type="text" placeholder="Menüde ara..."
          ...
          :style="{ 'focus:ring-color': settings.primary_color }" @input="handleSearch" />
    ...
    <div class="md:hidden sticky z-20 bg-white shadow-sm py-2 px-3"
      :class="showSearch ? 'top-[120px] sm:top-[124px]' : 'top-14 sm:top-16'">
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 md:py-10">
      <div v-if="loading" class="fixed inset-0 z-[9999] flex items-center justify-center bg-white/70 backdrop-blur-sm">
        <div class="flex flex-col items-center">
          <div class="relative w-16 h-16 sm:w-20 sm:h-20">
            ...
            <div class="absolute top-0 left-0 w-full h-full rounded-full border-4 border-transparent animate-spin"
              :style="{ borderTopColor: settings.primary_color }"></div>
          </div>
          <p class="mt-4 text-gray-700 font-medium text-sm sm:text-base">
            Menü yükleniyor...
          </p>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-8">
          <div v-for="item in searchResults" :key="item.id"
            ...
            <div class="relative overflow-hidden h-32 sm:h-40 flex-shrink-0">
              ...
              <div class="absolute top-2 right-2 px-2 py-1 rounded-full font-bold text-white shadow-md text-xs" :style="{
                background: `linear-gradient(135deg, ${settings.primary_color}, ${adjustColor(settings.primary_color, 40)})`
              }">
      ...
      <div v-else>
        ...
        <div>
          <div v-for="category in categories" :key="category.id" :id="category.id"
            class="mb-12 sm:mb-16 md:mb-24 scroll-mt-24 sm:scroll-mt-32 md:scroll-mt-40" data-aos="fade-up"
            data-aos-duration="1000">
            <div class="flex items-center mb-4 sm:mb-6 md:mb-10">
              <div class="p-2 sm:p-3 md:p-4 rounded-xl shadow-md mr-2 sm:mr-3 md:mr-5 flex items-center justify-center"
                :style="{
                ...
                <div class="text-xl sm:text-2xl md:text-3xl" :style="{ color: settings.primary_color }"
            ...
            <div v-else-if="getCategoryItems(category.id).length === 0"
              class="bg-white rounded-xl shadow-lg p-6 sm:p-8 md:p-12 text-center">
              <div
                class="inline-flex items-center justify-center p-3 sm:p-4 md:p-5 rounded-full bg-gray-100 mb-3 sm:mb-4 md:mb-5">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 text-gray-400"
                  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                    d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                </svg>
              </div>
              <p class="text-gray-500 text-sm sm:text-base md:text-lg font-medium">Bu kategoride henüz ürün
                bulunmamaktadır.</p>
            </div>
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-8">
              <div v-for="(item, index) in getCategoryItems(category.id)" :key="item.id"
                ...
                :data-aos-delay="(index % 3) * 100" data-aos-duration="800" data-aos-anchor-placement="top-bottom">
                ...
                <div class="relative overflow-hidden h-40 sm:h-48 md:h-56 flex-shrink-0">
                  <img v-if="item.image_url" :src="item.image_url" :alt="item.name"
                    ...
                    loading="lazy" />
                  <div v-else class="w-full h-full flex items-center justify-center bg-gray-50 text-gray-300">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 sm:h-12 sm:w-12 md:h-16 md:w-16"
                      fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                        d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                  ...
                  <div
                    ...
                    :style="{ '--tw-gradient-from': settings.primary_color + 'aa' }"></div>
                </div>
                ...
                <div class="p-3 sm:p-4 md:p-6 flex-grow flex flex-col">
                  ...
                  <div class="mb-2 sm:mb-3 md:mb-4">
                    <h3
                      class="text-base sm:text-lg md:text-xl font-bold text-gray-900 mb-1 md:mb-2 group-hover:text-indigo-600 transition duration-300 line-clamp-2"
                  ...
                  <div v-if="item.variations && item.variations.length > 0" class="mt-auto">
                    <div class="flex items-center mb-2">
                      <div class="w-1 h-4 sm:h-5 md:h-6 rounded-full mr-2"
                        :style="{ backgroundColor: settings.primary_color }"></div>
                      <p class="text-xs sm:text-sm font-semibold text-gray-700">Seçenekler</p>
                    </div>

                    <div class="space-y-1 sm:space-y-1.5 md:space-y-2">
                      <div v-for="(variation, index) in item.variations.slice(0, 3)" :key="index"
                        ...
                        <span class="px-2 py-1.5 sm:py-2 md:px-3 md:py-2.5 bg-gray-50 text-gray-700 flex-1 truncate">{{
                          getVariationName(variation) }}</span>
                        <span class="px-2 py-1.5 sm:py-2 md:px-3 md:py-2.5 font-medium flex-shrink-0"
