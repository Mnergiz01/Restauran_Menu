<template>
    <div class="bg-white shadow overflow-hidden sm:rounded-md">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="product in products" :key="product.id">
                            ...
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <img class="h-10 w-10 rounded-full object-cover"
                                        :src="product.image_url || '/placeholder.svg?height=40&width=40'" alt="" />
                                </div>
                                <div class="ml-4">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button @click="openEditModal(product)" class="text-indigo-600 hover:text-indigo-900 mr-4">
                                Düzenle
                            </button>
        ...
        <div v-if="showModal"
            class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div
                class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full max-h-full overflow-y-auto">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <form @submit.prevent="saveProduct">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="productCategory" v-model="currentProduct.category_id"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" id="productPrice" v-model="currentProduct.price"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                placeholder="0" min="0" required />
                        </div>

                        <div class="mb-4">
                            <label for="productImageUrl" class="block text-sm font-medium text-gray-700">Resim
                                URL</label>
                            <input type="text" id="productImageUrl" v-model="currentProduct.image_url"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        ...
                        <div class="mb-4">
                            ...
                            <div v-for="(variation, index) in variations" :key="index" class="flex mb-2">
                                <input type="text" v-model="variations[index]"
                        ...
                        <div class="mb-4">
                            <div class="flex items-center">
                                <input id="productActive" type="checkbox" v-model="currentProduct.active"
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    <strong>{{ productToDelete?.name }}</strong> ürününü silmek istediğinizden emin
...
async function fetchProducts() {
    loading.value = true
    try {
        await menuStore.fetchProducts(selectedCategoryId.value)
        products.value = menuStore.products
    } catch (error) {
...
async function saveProduct() {
    saving.value = true
    try {
        currentProduct.value.variations = variations.value
        if (isEditing.value) {
            await menuStore.updateProduct(currentProduct.value)
        } else {
            await menuStore.addProduct(currentProduct.value)
        }
        await fetchProducts()
        showModal.value = false
    } catch (error) {
...
async function deleteProduct() {
    deleting.value = true
    try {
        await menuStore.deleteProduct(productToDelete.value.id)
        await fetchProducts()
